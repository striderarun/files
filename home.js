...
jQuery(function ($) {
    ...
    CodeLearner.CreateProblem = {
        ...
        logoutUser: function () {
            var description = $('#problemDescription').val().trim()
            console.log('Inoput' + description)
            var login = {"@type": "login","username": "asubramonian_dsml","password": "asdf@1234"};
            var predict = {
                "product_description":"When you are looking for the ideal envelope to send your cherished photos in"
            }
            var that = this;
            that.postData("https://qa-ma.rel.infaqa.com/ma/api/v2/user/login", login)
                .complete(function (response) {
                    var data = JSON.parse(response.responseText);
                    var sessionId = data.icSessionId;
                    console.log(sessionId);
                    
                    that.postData("https://qa-pod1-dsml.rel.infaqa.com/ml-predict/api/v1/deployment/5mSP9a2bqG2epLyf5eIFYx", predict)
                        .complete(function (response) {
                            console.log('Output')
                            var data = JSON.parse(response.responseText);
                            console.log(data);
